...
def conectar_youtube_api():
    """
    Crear servicio de YouTube API usando variable de entorno
    ...
    if not api_key:
        st.error("No se encontró la API Key de YouTube. Verifica tu archivo .env")
        return None
...
def analizar_toxicidad(texto, clasificador, umbral=0.7):
    """Analiza la toxicidad de un texto"""
    try:
        resultado = clasificador(texto)[0]
        es_toxico = resultado['label'] == 'toxic' and resultado['score'] > umbral
        probabilidad = resultado['score'] if es_toxico else 0
        return es_toxico, probabilidad
    except Exception as e:
        st.error(f"Error en análisis de toxicidad: {e}")
        return None, None
...
def main():
    ...
    api_key = os.getenv('YOUTUBE_API_KEY')
    if not api_key:
        st.error("Por favor, configura tu YOUTUBE_API_KEY en el archivo .env")
        return
    ...
    with st.sidebar:
        ...
        2. Ajusta el número de comentarios
    ...
    if st.button("🔍 Analizar Comentarios") and url_video:
