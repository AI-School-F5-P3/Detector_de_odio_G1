...
def tokenize_text(text, tokenizer, max_length=256):
    """
    ...
    if st.button('Analyze'):
        ...
        if function_option == 'Analyze Comments':
            ...
            for comment in comments:
                hate_prediction = predict_hate_speech(comment, active_model, roberta_tokenizer)
                sentiment = sentiment_analyzer(comment)[0]
                
                st.write(f"Comment: {comment}")
                st.write(f"Hate Speech Prediction: {hate_prediction}")
                st.write(f"Sentiment: {sentiment['label']} (Score: {sentiment['score']:.2f})")

        elif function_option == 'Analyze Video Content':
            transcript = get_youtube_transcript(video_id)
            if transcript:
                hate_prediction = predict_hate_speech(transcript, active_model, roberta_tokenizer)
